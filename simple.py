...
parser.add_argument('--epoch', '-e', type=int, default=20000,
...
if args.rand:
    ...
    train_raw = get_raw_dataset(target_function, 0, 2 * math.pi, N)

test_raw = get_raw_dataset(target_function, 0, 2 * math.pi, NT)
...
for epoch in xrange(1, args.epoch + 1):
    ...
    for i in xrange(0, NT, test_bat):
        ...
        t = chainer.Variable(xp.asarray(y_test[i:i + test_bat]), volatile='off')
        model_loss = model(x, t)
        loss = float(model_loss.data)
        sum_loss += loss * test_bat
        num += test_bat
    # end for
    test_loss = sum_loss / num
    loss_test.append(test_loss)
    if (epoch % 10 == 0):
        print(json.dumps({'epoch': epoch, 'test loss': test_loss}))

    # minimum check
    if (test_loss < min_test_loss):
        print('epoch :', epoch, ', minimum test loss :', test_loss)
        min_test_loss = test_loss

    ##if (epoch == 35) or (epoch == 75):
    ##    optimizer.lr *= 0.1
...
plt.title('Test Error and Train Loss')
plt.legend()
# plt.ylim(0.0001, 0.8)
plt.yscale('log')
# plt.show()
...
for i in xrange(0, NT, test_bat):
    ...
    t = chainer.Variable(xp.asarray(y_test[i:i + test_bat]), volatile='off')
    model_loss = model(x, t)
    out_data[i:i + test_bat] = cuda.to_cpu(model.output.data)

out_data = out_data.reshape((NT,))
x_test = x_test.reshape((NT,))
y_test = y_test.reshape((NT,))
